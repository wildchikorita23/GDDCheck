   ...
   "evaluation_criteria": [
    {
      ...
      "evaluation_factors": [
        "주제가 명확하고 독창적인가?",
        "특정 게임이나 유저를 대상으로 하는지",
        "기획 의도가 분명한지 확인"
      ],
      "deduction_factors": [
        "창작 기획서인데 대상으로 삼은 게임이 없음",
        "주제가 너무 뻔함",
        "다양한 역량을 보여주기 어려운 주제",
    ...
    },
    {
      "name": "창의력",
      "evaluation_factors": [
        "기존 게임에서 볼 수 없는 창의적인 아이디어나 메커니즘이 있는가?",
        "독창적인 접근 방식이 돋보이는지"
      ],
      "deduction_factors": [
        "너무 뻔한 논리 전개 흐름",
        "너무 당연한 기획 의도 (예: 매출 증대, 리텐션 증가, 캐릭터 애착 형성)",
        "공통적으로 보이는 내용과 문서의 구성"
      ],
    ...
    },
    {
      "name": "분석력",
      "evaluation_factors": [
        "게임 메커니즘에 대한 이해가 깊고, 기획에 대한 분석이 잘 이루어졌는지",
                   ...
                   # 각 분할된 텍스트에 대해 평가 수행
                    for split_text in split_texts:
                      # 프롬프트를 평가 기준을 포함하여 OpenAI API 호출
                       evaluation_prompt = (
    f"다음은 게임 기획서 평가 기준입니다. 각 평가 항목을 10점 만점으로 평가하고, 각 항목의 점수와 평가 멘트를 "
    f"아래 형식과 설명에 따라 구체적으로 작성해 주세요:\n\n"

    f"### 평가 항목\n"
    f"1. 주제 선정\n"
    f"- 평가 요소: 주제가 명확하고 독창적인지, 특정 게임이나 유저를 대상으로 하는지, 기획 의도가 분명한지.\n"
    f"- 감점 요소: 창작 기획서인데 대상으로 삼은 게임이 없음, 주제가 너무 뻔함, 다양한 역량을 보여주기 어려운 주제.\n\n"

    f"2. 창의력\n"
    f"- 평가 요소: 기존 게임에서 볼 수 없는 창의적인 아이디어나 메커니즘이 있는지, 독창적인 접근 방식이 돋보이는지.\n"
    f"- 감점 요소: 뻔한 논리 전개, 당연한 기획 의도, 평범한 구성.\n\n"

    f"3. 구성과 흐름\n"
    f"- 평가 요소: 문서가 논리적 순서에 따라 잘 구성되어 있는지, 각 섹션이 유기적으로 연결되어 있는지.\n"
    f"- 감점 요소: 찾기 어려운 내용, 뒤죽박죽인 구성, 중복된 내용.\n\n"

    f"4. 가독성\n"
    f"- 평가 요소: 텍스트가 읽기 쉽고, 시각적 구성 요소가 적절히 사용되었는지, 레이아웃이 정돈되어 있는지.\n"
    f"- 감점 요소: 불필요한 공백, 사진 배경, 가독성 낮은 폰트.\n\n"

    f"5. 문장력과 맞춤법\n"
    f"- 평가 요소: 문장이 간결하고 명확한지, 맞춤법과 문법 오류가 없는지.\n"
    f"- 감점 요소: 길고 복잡한 문장, 주어와 술어 불일치, 맞춤법 오류.\n\n"

    f"6. 분석력\n"
    f"- 평가 요소: 게임 메커니즘에 대한 깊은 이해와 분석, 데이터나 사례를 통한 설득력 있는 설명.\n"
    f"- 감점 요소: 겉핥기식 분석, 게임 메커니즘 이해 부족.\n\n"

    f"7. 논리력\n"
    f"- 평가 요소: 시스템과 메커니즘이 논리적이고 일관성 있게 제시되었는지, 설득력 있는 설명 포함.\n"
    f"- 감점 요소: 논리적 비약, 설득력 부족한 근거.\n\n"

    f"8. 통찰력\n"
    f"- 평가 요소: 기획서가 게임의 문제를 파악하고 해결책을 제시하는지, 문제 해결력과 실행 가능성 평가.\n"
    f"- 감점 요소: 뻔한 내용, 문제 해결력 부족.\n\n"

    f"9. 실무력\n"
    f"- 평가 요소: 실제 게임 개발 환경에서 적용 가능한 실용적인 기획 요소가 포함되었는지.\n"
    f"- 감점 요소: 플로우차트에 설명 부족, 테이블 설계 의도 미작성.\n\n"

    f"10. 시각적 구성 및 디자인\n"
    f"- 평가 요소: 문서의 시각적 디자인이 가독성을 높이고, 정보 전달을 효과적으로 돕는지.\n"
    f"- 감점 요소: 일관되지 않거나 조잡한 디자인.\n\n"

    f"### 평가 형식\n"
    f"- 각 항목별로 점수(0-10점)와 함께 3-5줄의 구체적인 평가 멘트를 작성하세요.\n"
    f"- 점수는 해당 항목의 수준을 반영해야 하며, 평가 멘트는 강점과 약점을 구체적으로 언급하세요.\n\n"

    f"### 평가 예시\n"
    f"주제 선정: 7점\n"
    f"주제는 명확하지만 기존에 자주 사용된 컨셉이라 독창성이 떨어집니다. 특정 유저 타겟팅이 부족해 보입니다.\n\n"
    
    f"창의력: 6점\n"
    f"몇 가지 창의적인 요소가 있으나, 전체적으로 새로운 접근 방식이 부족합니다. 기획 의도가 평범합니다.\n\n"
