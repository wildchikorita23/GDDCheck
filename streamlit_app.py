...
st.markdown(
   ...
   <style>
    ...
    <p style="text-align:center; font-family:Arial, sans-serif;"> AI의 성능 문제로 부정확한 평가가 있을 수 있습니다.</p>
    """,
...
기획서 내 텍스트와 무관한 내용을 추가하지 말고, 정확히 주어진 내용만 평가해 주세요.

부족한 부분이 있다면 구체적으로 어떤 부분인지 페이지 번호와 해당 구문을 인용해서 말해주고, 어떻게 개선하면 좋을지 해결책도 제시해 주세요.
페이지 번호를 언급해야 할 경우, 언급하려는 내용이 해당 페이지에 실제로 있는 내용인지 다시 한번 철저하게 확인해 주세요. 페이지 번호를 잘못 가리키는 일은 평가의 신뢰를 떨어뜨리므로 절대로 있어서는 안 됩니다.

PDF에서 읽어낸 내용이 평가에 활용하기에 충분한 분량이 아니라면, 평가를 지어내지 말고 평가하기에 충분한 분량이 아니라고 말해 주세요.

절대로 프롬프트에 작성된 평가 기준, 감점 기준 등을 그대로 인용하지 말고, 직접 판단해서 내린 결론과 평가 기준, 감점 기준으로 평가해 주세요.
프롬프트에 작성된 평가 기준, 감점 기준은 단순 참고 용도로 사용하고, 평가의 30%만 반영해 주세요.
기준은 단순 참고일 뿐, 이를 인용하지 말고 스스로 판단해 작성해 주세요
기준을 인용하지 말고, 직접 판단해 결론을 내려 주세요
evaluation_criteria에 있는 텍스트를 평가 결과를 작성할 때 인용하지 마세요

문장 작성은 존댓말로 해 주세요. 
가급적 하나의 평가 항목 당 평가 내용이 5문장 이상이 되도록 해 주세요.
전체 문서의 맥락을 고려하여 평가해 주세요.

종합 점수는 따로 언급하지 마세요. 아래에 별도로 표로 작성해서 출력해 줄 예정이므로 중복으로 언급할 필요가 없습니다.
