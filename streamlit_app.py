   ...
   "evaluation_criteria": [
    ...
    },
    {
      ...
      "evaluation_factors": [
        ...
        "제안한 시스템, 콘텐츠의 Values at Play가 뚜렷하게 제시되고 있는가"
      ],
      "deduction_factors": [
        "논리적 비약 (예: 아무런 근거 없이 이 시스템을 도입하면 매출이 상승할거라 기대하는 것)",
        "설득력이 부족한 근거"
      ]
    },
    {
    ...
    },
    {
      ...
      "evaluation_factors": [
        ...
        "UI 구성 요소를 설명할 때, PPT나 FIGMA 툴을 이용해 그레이 박스로 직접 그린 UI 프로토타입 시안을 사용 (캐릭터 창작 기획서, BM 기획서, 이벤트 기획서는 해당하지 않음, 시스템 기획서와 시스템 역기획서에만 해당하는 내용)"
      ],
      "deduction_factors": [
        "플로우차트에 텍스트 설명이 없는 것",
...
절대로 프롬프트에 작성된 평가 기준, 감점 기준 등을 그대로 인용하지 말고, 직접 판단해서 내린 결론과 평가 기준, 감점 기준이 잘 어우러지도록 자연스럽게 녹여내 주세요.
부족한 부분이 있다면 구체적으로 어떤 부분인지 페이지 번호와 해당 구문을 인용해서 말해주고, 어떻게 개선하면 좋을지 해결책도 제시해 주세요.
페이지 번호를 언급해야 할 경우, 언급하려는 내용이 해당 페이지에 실제로 있는지 다시 한번 철저하게 확인해 주세요. 페이지 번호를 잘못 가리키는 일은 평가의 신뢰를 떨어뜨리므로 절대로 있어서는 안 됩니다.
문장 작성은 존댓말로 해 주세요. 가급적 하나의 평가 항목 당 평가 내용이 5문장 이상이 되도록 해 주세요.
